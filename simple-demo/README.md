...
Dockerfile is used to build the docker image for metadata-setup container.  The content of Dockerfile is quite simple: it uses alpine 3.12 as base image, install *curl* utility, 
...
The entrypoint script, **setup.sh**, hosts the logic to set up the metadata service.  In this simple example, **setup.sh** will issue several REST calls to metadata service 
...
### 1. Create app definition for the simple demo
...
```
### 2. Deploy the simple-demo app on the target Edge Builder node
Please use edgebuilder-cli command below to deploy the app *simple-demo* to the target node:
```
edgebuilder-cli app create -d simple-demo -n <target node>
```
### 3. Start the simple-demo app on the target Edge Builder node
Please use edgebuilder-cli command below to start the app on the target node:
```
edgebuilder-cli app start -a <app id>
```
### 4. Verify events exported to HiveMQ
